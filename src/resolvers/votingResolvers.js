...
export const votingResolvers = {
  Query: {
    // Placeholder for future voting queries
  },
  Mutation: {
    async submitVote(_, { movie1Id, movie2Id, winnerId }, context) {
      const token = context.token;
      console.log('Voting context:', token.replace('Bearer ', ''));
      if (!token) {
      ...
      if (loserMovie) {
        ...
        await moviesCollection.updateOne(
          ...
          {
            $set: {
              winningPercentage: Math.round(loserPercentage * 100) / 100
            }
          }
        );
      }
